...
A playback policy that keeps any two participating Media Devices within 250 ms of visual alignment rather than providing frame-accurate video-wall synchronization. It applies automatically to the capable Media Devices within one Channel (a legacy member that has not reported support keeps the additive playback contract and leaves the Channel `Sync Degraded`); a Channel Group shares the synchronization boundary only when an operator explicitly enables it. `docs/adr/0041-server-anchored-synchronized-playback.md`.
_Provisional_: Pending final acceptance of ADR 0041.
_Avoid_: Simultaneous delivery, frame sync

**Sync Degraded**:
The synchronization-health state where playback continues but the 250 ms pairwise alignment target cannot be guaranteed because one or more participating Media Devices is offline, not ready, unsupported, or no longer time-aligned with the server. It is separate from Channel Health's `Degraded` aggregation. `docs/adr/0041-server-anchored-synchronized-playback.md`.
_Provisional_: Pending final acceptance of ADR 0041.
_Avoid_: Offline (other Media Devices may still be online and aligned), Channel Health, playback failure

**Channel Group**:
A named, reusable collection of Channels and optional synchronization boundary, but not a Publication target. A Channel may belong to multiple Channel Groups, but may be claimed by at most one group's `Preparing`, `Enabled`, or `Disabling` synchronization transition; Publications continue to target Channels. `docs/adr/0041-server-anchored-synchronized-playback.md`.
_Provisional_: Pending final acceptance of ADR 0041.
_Avoid_: Sync Group (synchronization is an optional policy, not the group's identity), Channel Category
