...
**Asset** (Communication):
A reusable media file (image or video) stored in the central repository. Not video-specific — MVP formats are JPG, PNG, and MP4 (H.264). "Archive" hides it from pickers without removing it. "Delete" is a true hard-delete and is blocked outright if the Asset is referenced by any Active or Scheduled Publication — the operator must stop/cancel or wait out those Publications first — **and, once Publication snapshots exist (`docs/adr/0045-publication-snapshot-materialization.md` §10), also blocked permanently for any Asset that has ever been materialized into a Publish Job snapshot**, because deleting it would destroy the broadcast record. Archive remains the reversible action; waiting out the Publication no longer helps. Deleting a Publication, by contrast, still cascades its Jobs and their snapshots — that is unchanged.
...
**Folder**:
A flat, tenant-wide label for filing content. One namespace shared by Assets, Playlists and Layouts — the same Folder holds all three, and each list page shows only its own type, so the count beside a Folder is per page and legitimately differs between them. A content item belongs to **at most one** Folder; unfiled (`NULL`) is normal and is where every item starts. There is no nesting and no Trash: a Folder that still holds anything cannot be deleted, and an empty one is simply deleted. Names are unique per tenant. `docs/adr/0046-content-folders.md`.
_Avoid_: Tag (many-to-many, a different question and not built), sub-folder / folder tree (deliberately flat), `asset_folders` (that is Asset Intelligence's physical-asset table, unrelated)
...
**Layout**:
A named, reusable screen composition of up to four non-overlapping **Zones**, each expressed as a percentage of the display area. It carries geometry and zone roles only — never content: content is bound per Zone when an operator builds a Publication, so Publication remains the only place that decides what plays where and when. Zones may not overlap (there is no stacking order) and need not cover the whole display; uncovered area is the Layout's background colour. It declares an `aspect_ratio` that its percentages are measured against, and a background colour for uncovered area. Fit against a Media Device is its **own** rule, not the Channel one: a Layout declares an aspect ratio rather than a resolution, so the check is orientation and aspect-ratio compatibility — mismatch blocks — and a Device that has never been profiled fails the check rather than passing it. The Channel ↔ Device resolution rule (mismatch warns) is unchanged and separate. Lifecycle copies Playlist's: `active ↔ inactive`, no delete. A Publication that uses a Layout materializes both the geometry and every Zone's items into its snapshot, so editing the Layout never reshapes a screen that is already airing — which is why Layout depends on `docs/adr/0045-publication-snapshot-materialization.md` landing first. Because one poll response merges every in-window equal-priority Publication into a single loop, activating a Publication is **blocked** when it overlaps an equal-priority Publication on a shared Media Device and either side uses a Layout; higher-priority preemption still takes the whole screen. Publishing is also refused to any Media Device that has not reported the `multi_zone_v1` capability, or reports fewer concurrent video Zones than the Layout needs. Release one is single-screen only — multi-monitor video walls are deferred. `docs/adr/0044-multi-zone-layout.md`; player contract: `docs/layouts/contract-v2-zones.md`.
_Avoid_: Scene (the prior Aurora player nested a carousel of Scenes inside a Layout — deliberately not ported, because a Scene's own duration is scheduling and Schedule already owns that), Grid Layout / Custom Layout (superseded wording), Template (the starting compositions are frontend constants, not an entity)

**Zone**:
One rectangular region of a Layout, with a `role` (`main`, `sidebar`, `ticker`, `secondary`), its own content and playback loop, and its own playback settings — play mode, repeat and start position belong to a Zone, so a looping main Zone can sit beside a play-once ticker. A Publication with no Layout still has exactly one implicit Zone covering the whole screen, so nothing in the system has to special-case "no zones". Zone loops run independently and need not share a duration — Synchronized Playback's phase arithmetic (`docs/adr/0043-schedule-anchored-playback-loop.md`) is evaluated per Zone against that Zone's own loop, using the one `loop_anchor_at` the response carries for all of them, so a synchronized Channel stays aligned in every Zone. A higher-priority Publication suppresses the entire screen rather than preempting a single Zone, so Schedule and Publication Priority stay whole-screen concepts. Proof-of-play records the **snapshot** Zone id, not the Layout's — a Layout can be edited after publish, so only the snapshot identifies the geometry that actually aired; full-screen playback leaves it null.
