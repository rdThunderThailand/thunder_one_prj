...
**Frontier right now:** #63/#64/#65/#66/#67/#120 are all CLOSED (2026-09-17). **Epic build is
done.** Two loose ends only: PR #74 (the #67 backfill migration) is still Draft pending a
browser/UI verify on prod's Media Detail page (blocked on a login this session doesn't have
credentials for — see the SESSIONLOG); and the held #121. Develop was not backfilled (4 of its 6
refused Assets are airing — the unverified #66 real-player leg); if that is ever wanted, rehearse
it there with a real player, never on prod first.
**Order:** ~~63 → (64 ∥ 65) → 66~~ done → ~~120~~ done → ~~67 (prod only)~~ done. Only #74's Draft
flip and #121 remain. 121 is independent.
