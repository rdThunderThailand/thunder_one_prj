...
**Frontier right now:** #63/#64/#65/#66/#67/#120 are all CLOSED (2026-09-17), and PR #74 (the #67
backfill migration) is fully verified — SQL row counts, RPC, and a real prod browser check of
Media Detail all pass. **Epic is entirely done except a human clicking PR #74 to Ready** (the
assistant never flips Draft→Ready itself) and the held #121. Develop was not backfilled (4 of its
6 refused Assets are airing — the unverified #66 real-player leg); if that is ever wanted, rehearse
it there with a real player, never on prod first.
**Order:** ~~63 → (64 ∥ 65) → 66~~ done → ~~120~~ done → ~~67 (prod only)~~ done, verified. Only
#74's Ready flip (human) and #121 remain. 121 is independent.
