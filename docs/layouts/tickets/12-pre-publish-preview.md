...
- [x] One component takes a list of Zones — geometry, items, per-item duration and transition,
      playback settings — which is the shape ADR 0045 §1 defines and the shape the player receives
- [x] A Playlist is previewed as **one Zone** at `x=0 y=0 w=100 h=100`. There is no separate
      playlist-preview mode to keep in step with the Zone one
- [x] It opens as a full-screen modal locked to the Layout's aspect ratio, not animated inside the
      editing canvas — content moving while someone drags a Zone edge is a distraction
- [x] **Every Zone's state is a pure function of one shared `t`**: `t mod` that Zone's own loop length
      gives its current item index and the offset into it. A timer per Zone makes the scrubber a
      rewrite instead of an addition
- [x] Zones loop independently from a common start at `t = 0`, matching `loop_anchor_at` being one
...
- [x] Loop length resolves duration the way activation does — `COALESCE(item, asset)`. Any other rule
      makes the number of restarts wrong, which is the one thing this has to get right
- [x] An item that resolves to no duration is drawn as a marked placeholder and contributes no time. A
...
- [x] Unapproved or missing assets are drawn as marked placeholders carrying the asset name, not
      hidden — the operator needs to see why something will not air
- [x] A timeline scrubber sets `t`; 2× and 4× speed change how fast `t` advances (`<video>` already has
      `playbackRate`)
- [x] Decoding capacity is not simulated and is not enforced during the current publish phase. The
      ...
      either)
- [x] When step 5 has detected a schedule conflict — `computeEligibility` already receives `conflicts`
...
- [x] The merged result is not simulated — resolving overlap windows and priorities in the browser is a
      second scheduler
- [x] One `*.check.mts` on the clock: item index and offset at several `t` across two Zones of
      different lengths, including a zero-length Zone
- [x] No new dependency
- [x] Verified in the browser from all three mount points
