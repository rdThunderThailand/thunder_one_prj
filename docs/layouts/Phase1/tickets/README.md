...
| 23 | [#53](https://github.com/rdThunderThailand/thunder_one_prj/issues/53) | [API route for `set_tags`](23-composition-tags-route.md) | Core | 22 | in progress (code done, HTTP-verify after deploy) |
...
| 29 | [#59](https://github.com/rdThunderThailand/thunder_one_prj/issues/59) | [List page rails](29-list-page-rails.md) | One | 23 | not started |
...
- **26 ‖ 27 ‖ 28** — all three edit the editor, so **25 must land first**: it splits
  `CompositionEditorPage.tsx` (734 lines) into separate files, which is what makes the three
  parallel instead of three merge conflicts.
...
- `CREATE OR REPLACE FUNCTION` does **not** replace when a parameter is added; it creates an
  overload and breaks every existing call. Nothing in this phase adds a parameter. Keep it that way.
