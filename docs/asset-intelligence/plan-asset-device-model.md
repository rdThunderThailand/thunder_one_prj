...
- **Option A**: `ai-assets`'s `Asset.externalRef` stores the `media_core` Device/`channel_devices` id. Keeps the pointer next to the entity whose lifecycle drives the assignment (Asset Intelligence decides "this Asset now serves as a Device").
