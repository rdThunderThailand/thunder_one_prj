...
export function AssetTable({ assets, trash, folders, onRefresh, previewUrls, thumbnailUrls }: {
  assets: MediaAsset[];
  trash: boolean;
  ...
  return <div className="overflow-x-auto">
    <table className="w-full min-w-[900px] table-fixed text-left text-sm">
      <thead><tr className="border-b border-zinc-100 text-xs font-medium text-zinc-500 dark:border-zinc-800 dark:text-zinc-400">
        <th className="w-[72px] py-2 pl-1">Preview</th>
      ...
      <tbody>{assets.map((asset) => {
        ...
        return <tr key={asset.id} className="border-b border-zinc-100 last:border-0 dark:border-zinc-800">
          <td className="py-3 pl-1"><Link href={`/media-workspace/assets/${asset.id}`} aria-label={`View ${label}`}><MediaThumb url={previewUrls[asset.id]} thumbnailUrl={thumbnailUrls[asset.id]} kind={asset.kind} mimeType={asset.file?.mime_type} alt={label} className="h-10 w-16 rounded-md" /></Link></td>
