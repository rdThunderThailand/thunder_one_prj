...
export function CompositionPickerModal({ selectedId, onClose, onSelect }: { selectedId: string | null; onClose: () => void; onSelect: (id: string) => void }) {
  ...
  return (
    <Modal open onClose={onClose} title="Layout Picker" size="preview" showCloseButton footer={footer}>
      <p className="-mt-2 text-xs text-muted-foreground">เลือก Layout ที่ต้องการใช้งาน</p>
      <div className="mt-1 flex items-center gap-2 border-y border-border py-3">
        ...
        <Link href="/media-workspace/layouts" target="_blank" className="inline-flex items-center gap-1 px-2 py-2 text-sm font-medium text-primary">
      ...
      <div className="grid h-[min(42rem,calc(100vh-15rem))] min-h-[30rem] grid-cols-[11rem_minmax(0,1fr)_16rem] overflow-hidden border-b border-border">
          ...
          <PickerFilterSection label="Orientation" divided>
            <div role="radiogroup" aria-label="Orientation">
              ...
              <PickerFilterChoice
                ...
                label="Landscape"
                marker={<MonitorIcon className="h-3.5 w-3.5 text-muted-foreground" />}
                onClick={() => update({ orientation: "landscape" })}
              />
              <PickerFilterChoice
                checked={filters.orientation === "portrait"}
                label="Portrait"
                marker={<MonitorIcon className="h-3.5 w-3.5 rotate-90 text-muted-foreground" />}
        ...
        <section className="flex min-w-0 flex-col overflow-hidden p-4">
          <div className="mb-3 flex items-center justify-between">
            <p className="text-xs font-medium text-muted-foreground">{items === null ? "Loading layouts…" : error ?? `${sorted.length} layouts found`}</p>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto pr-1">
            ...
            {items !== null && !error && !visible.length && <p className="py-12 text-center text-sm text-muted-foreground">No layouts match these filters.</p>}
          </div>
        ...
        <aside className="overflow-y-auto border-l border-border">
          {selected ? (
            ...
            <p className="p-5 text-sm text-muted-foreground">เลือก Layout เพื่อดูรายละเอียด</p>
          )}
        </aside>
      </div>
    </Modal>
        ...
        <ul className="mt-2 space-y-1 rounded-lg bg-muted p-3">
          {detail.zones.map((zone) => (
            <li key={zone.layout_zone_id} className="flex justify-between gap-3 text-xs">
              <span className="truncate text-muted-foreground">{zone.position + 1}. {zone.name}</span>
              <span className="shrink-0 text-muted-foreground">{zone.playlist_id ? "Bound" : "Unbound"}</span>
            </li>
          ))}
        </ul>
      ) : (
  ...
  return (
    <button type="button" onClick={onSelect} className={`overflow-hidden rounded-xl border text-left ${selected ? "border-primary ring-2 ring-primary" : "border-border"}`}>
      ...
      <div className="space-y-1 p-2">
        ...
        <p className="text-[10px] text-muted-foreground">{compositionLayoutDisplayName(item)} · {item.zone_count} zones</p>
  ...
  return (
    ...
    >
      ...
      <span className="text-right">
        <Badge color={badge.color} variant="pill">{badge.label}</Badge>
        <span className="mt-1 block truncate text-[10px] text-muted-foreground">{item.createdBy?.displayName ?? "—"}</span>
      </span>
    </button>
  );
}
