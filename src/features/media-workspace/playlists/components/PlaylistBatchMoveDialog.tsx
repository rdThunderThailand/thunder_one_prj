...
export function PlaylistBatchMoveDialog({ open, ids, folders, onClose, onDone, onError }: {
  open: boolean;
  ids: string[];
  ...
  const submit = async () => {
    setBusy(true);
    try {
      const results = await Promise.allSettled(ids.map((id) => movePlaylist(id, folderId || null)));
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => { if (!next && !busy) onClose(); }}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Move {ids.length} playlist{ids.length === 1 ? "" : "s"}</DialogTitle>
          <DialogDescription>Choose a destination folder for the selected playlists.</DialogDescription>
        </DialogHeader>
