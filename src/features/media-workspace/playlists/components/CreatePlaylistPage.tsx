...
export function CreatePlaylistPage() {
  ...
  if (resumeError) {
    return (
      <Card className="p-6">
        ...
        <Button className="mt-4" variant="secondary" onClick={() => router.push("/media-workspace/playlists")}>
          กลับไป Playlists
        </Button>
      </Card>
    );
  ...
  const goBack = () => {
    ...
    router.push("/media-workspace/playlists");
  };
      ...
      {confirmLeave && (
        <Card className="flex items-center justify-between gap-4 border-amber-200 p-4 dark:border-amber-900">
          ...
          <span className="flex shrink-0 gap-2">
            ...
            <Button variant="ghost" onClick={() => router.push("/media-workspace/playlists")}>
              ออกโดยไม่บันทึก
            </Button>
          </span>
        </Card>
