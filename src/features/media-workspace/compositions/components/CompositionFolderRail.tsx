...
import { ContentFolderRail, type FolderCollection } from "@/features/media-workspace/content-library/ContentFolderRail";
import { isDescendant } from "@/features/media-workspace/content-library/folder-tree";
import {
  createContentFolder,
  deleteContentFolder,
  moveContentFolder,
...
export function CompositionFolderRail({
  folders,
  selected,
  onSelect,
  onRefresh,
  ...
  const create = () => run(() => createContentFolder("composition", { name: value.trim(), parent_id: selectedFolderId }));
  const submitAction = () => {
    if (!action) return;
