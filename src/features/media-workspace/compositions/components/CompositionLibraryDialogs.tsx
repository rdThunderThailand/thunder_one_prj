...
export function CompositionLibraryDialogs({
  ...
  if (action === "trash" && showPrograms) {
    return <Modal open={target !== null} onClose={onClose} title={title} size="lg" showCloseButton footer={<Button type="button" variant="secondary" onClick={onClose}>Close</Button>}>
      <p className="-mt-2 mb-2 text-muted-foreground">This layout is used by {programs?.length ?? 0} program{programs?.length === 1 ? "" : "s"}.</p>
      <div className="overflow-hidden rounded-lg border border-border">
        <table className="w-full text-left text-sm"><thead className="bg-muted text-xs text-muted-foreground"><tr><th className="px-3 py-2 font-medium">Program name</th><th className="px-3 py-2 font-medium">Status</th><th className="px-3 py-2 font-medium">Schedule</th></tr></thead>
          <tbody>{programs?.map((program) => <tr key={program.id} className="border-t border-border"><td className="px-3 py-2 font-medium text-foreground"><Link href={program.status === "draft" ? `/media-workspace/publications/create?id=${program.id}` : `/media-workspace/publications/${program.id}`} onClick={onClose} className="hover:text-primary hover:underline">{program.name}</Link></td><td className="px-3 py-2"><span className={`rounded-full px-2 py-0.5 text-xs font-medium ${program.status === "active" ? "bg-success-soft text-success" : program.status === "scheduled" ? "bg-warning-soft text-warning" : "bg-muted text-muted-foreground"}`}>{program.status[0].toUpperCase() + program.status.slice(1)}</span></td><td className="px-3 py-2 text-muted-foreground">{formatProgramSchedule(program)}</td></tr>)}</tbody>
        </table>
      </div>
    </Modal>;
  }
  ...
  return <Modal open={action !== null && target !== null} onClose={onClose} title={title} hideTitle={action === "trash"} showCloseButton={action === "trash"} footer={<>
    ...
    {action === "trash" && <div className="space-y-3 text-center">
      <span className={`mx-auto grid h-12 w-12 place-items-center rounded-full ${isTrashBlocked ? "bg-warning-soft text-warning" : "bg-danger-soft text-danger"}`} aria-hidden="true">{isTrashBlocked ? <WarningIcon /> : <TrashIcon />}</span>
