  ...
  return (
    <Card className="flex min-h-74 flex-col p-4">
      ...
      <div className="mb-5 flex items-center gap-4">
        {isNow ? <div className="flex h-28 w-28 shrink-0 items-center justify-center rounded-lg bg-slate-950 text-cyan-100"><BroadcastIcon className="h-9 w-9" /></div> : <time className="border-r border-zinc-100 pr-5 text-2xl font-semibold text-zinc-900 dark:border-zinc-800 dark:text-zinc-50">{nextOpensAt ? formatTime(nextOpensAt) : "—"}</time>}
        <div className="min-w-0"><h3 className="truncate font-semibold text-zinc-900 dark:text-zinc-50">{publication.name}</h3><p className="mt-1 text-xs capitalize text-zinc-500">{publication.publication_type}</p><p className="mt-3 text-xl font-semibold text-zinc-900 dark:text-zinc-50">{targetSummary(status)}</p></div>
      </div>
