...
- `mock-data.ts` — `mockWorkOrders` (spans several dates, not just "today"), `todaySummary` (derived from it), and `mockTechnicians` (read by `ai-service-ops`'s Work Queue dispatch picker). `WorkOrder.issueId` links a subset of these back to `ai-issues`'s `Issue.id` — see that feature's README for why this link exists and how it was verified.
