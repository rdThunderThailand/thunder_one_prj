...
import {
  managerWorkspaceCategories,
  managerWorkspaceTiles,
  type ManagerWorkspaceIcon,
  type ManagerWorkspaceRole,
  type ManagerWorkspaceTileData,
} from "../mock-data";
...
function WorkspaceTile({ tile }: { tile: ManagerWorkspaceTileData }) {
  const body = (
    <>
...
export function ManagerWorkspaceDirectory() {
  const [category, setCategory] = useState<(typeof managerWorkspaceCategories)[number]>("All");
  const [view, setView] = useState<"grid" | "list">("grid");

  const tiles =
    category === "All" ? managerWorkspaceTiles : managerWorkspaceTiles.filter((t) => t.category === category);
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-1.5">
          {managerWorkspaceCategories.map((cat) => (
            <button
              key={cat}
              type="button"
          ...
          <button
            type="button"
            title="Not built yet"
            className="flex h-8 w-8 cursor-not-allowed items-center justify-center rounded-lg text-zinc-400"
          >
