...
The CEO / Owner's dashboard for Asset Intelligence — organization-wide asset risk and value overview (landing page at `/mission-control`). Composes `components/ui` primitives (`Card`, `Sparkline`, `ProgressBar`), same pattern as `features/overview`'s dashboard.

`mock-data.ts` derives its numbers from `asset-intelligence/assets`'s `getMockAssets()`/`mockDepartments` rather than inventing separate fake data — there's no real insights/recommendation backend yet (see requirement doc §4.1 CEO-01..05).
...
- `components/` —
  ...
  - `InsightsPage` (CEO-05) — trends and cross-department benchmarks (reads `asset-intelligence/assets`'s `mockDepartments`)
...
- `mock-data.ts` — `statCards`, `recentAlerts`, `mockRecommendations`, `assetOutlook` — all derived from `asset-intelligence/assets` where possible, no backend yet
