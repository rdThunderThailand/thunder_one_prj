...
`mock-data.ts` derives its numbers from `ai-assets`'s `getMockAssets()` rather than inventing separate fake data — there's no real insights/recommendation backend yet (see requirement doc §4.1 CEO-01..05 and `docs/adr/0024-asset-device-cross-reference-model.md`).
