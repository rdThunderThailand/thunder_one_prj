...
export async function getSession(): Promise<SessionResult> {
  ...
  try {
    ...
    return { userName: FALLBACK_NAME, tenantName: null, ...NO_ROLE };
  }
  ...
  if (!user) {
    return { userName: FALLBACK_NAME, tenantName, ...role };
  }

  return { userName: resolveUserName(user), tenantName, ...role };
}
