...
import { transferPageSize, transferRows, transferTotalCount, transferTotalPages } from "../mock-data";

export function TransferTableControls() {
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-4">
        <span className="text-sm text-zinc-400">
          1-{transferRows.length} จาก {transferTotalCount.toLocaleString()} รายการ
        </span>
        <div className="flex items-center gap-1">
          ...
          {Array.from({ length: transferTotalPages }, (_, i) => i + 1).map((page) => (
            <button
              key={page}
              type="button"
              title="Not built yet"
