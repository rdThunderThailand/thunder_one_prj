...
import { warrantyFilterOptions } from "../mock-data";
    ...
    <Card className="flex flex-col gap-3 p-4">
      <div className="flex items-center gap-2 rounded-lg border border-zinc-200 px-3 py-2 text-sm text-zinc-400 dark:border-zinc-700">
        ...
        <span className="flex-1 truncate" title="Not built yet">
          ค้นหา Asset, Serial, รุ่น, Vendor...
        </span>
      </div>
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-5">
        <FilterSelect options={warrantyFilterOptions.category} />
        <FilterSelect options={warrantyFilterOptions.status} />
        <FilterSelect options={warrantyFilterOptions.vendor} />
        <FilterSelect options={warrantyFilterOptions.expiringWithin} />
        <span
          title="Not built yet"
